...
const MusicManagementScreen = () => {
    ...
    const asyncGetMusic = async () => {
        ...
        if(response.status === 200) {
            setListMusic(response.data.data);
            setListRenderMusic(response.data.data);
        }
    }
    ...
    // lọc theo tìm kiếm
    const onChangeSearchMusic = (e) => {
        e.preventDefault();
        const key = removeVietnameseTones(e.target.value).toLowerCase();
        if(key !== "") {
            const ret = listMusic.filter((music) => {
            ...
            setListRenderMusic(ret);
        }
        else {
            setListRenderMusic(listMusic);
        }
    }
    ...
    const indexOfLastPost = currentPage * postsPerPage;
  	...
  	const currentList = listRenderMusic.slice(indexOfFirstPost, indexOfLastPost)
        ...
        <div className={classnames("admin-page", "music-admin-page")}>
            ...
            <div className={classnames("music-management")}>
                ...
                <div className={classnames("music-management-action", "mg-b-10")}>
                    <div>
                        <input placeholder="Tìm kiếm bài hát" onChange={onChangeSearchMusic}/>
                        ...
                        <span className="mg-l-20">Tổng số: { listRenderMusic.length }</span>
                    </div>
                        ...
                        <tbody>
                            { currentList.map((music, index) => {
                                ...
                                <Pagination
                                    ...
                                    indexOfFirstSong={indexOfFirstPost}
                                    songsPerPage={postsPerPage}
                                    totalSongs={listRenderMusic.length}
                                    paginate={paginate}
                                />
                        </tbody>
                        :
